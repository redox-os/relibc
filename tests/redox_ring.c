...
int main()
{
    ...
    printf("Initializing ring on path '%s'...\n", disk_path);
    fflush(stdout);

    int ret = redox_ring_queue_init_with_path(
        entries, sqe_size, cqe_size, pool_size, chunk_size, 0, ring, 0, disk_scheme_root);
    ...
    for (int i = 0; i < total_ops; i++) {
        buf_offset = redox_ring_alloc_buf(ring, &buf);
        while (buf_offset < 0) {
            if (unsubmitted > 0) {
    ...
    printf("\nStarting Read Benchmark (with async batching & verify)...\n");
    fflush(stdout);
    clock_gettime(CLOCK_MONOTONIC, &start);
    ...
    for (int i = 0; i < total_ops; i++) {
        ...
        while (buf_offset < 0) {
            ...
            redox_ring_free_buf(ring, completed_buf_offset);
            in_flight--;
            buf_offset = redox_ring_alloc_buf(ring, &buf);
        }
