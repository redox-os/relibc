...
fn get_mem_policy(policy: &mut [u8]) -> Result<()> {
...
fn set_mem_policy(policy: &[u8]) -> Result<()> {
    let thread_fd = redox_rt::RtTcb::current().thread_fd();
    let addrspace_fd = thread_fd.dup(b"addrspace")?;
...
pub unsafe extern "C" fn redox_sys_call_v0(
    ...
    Error::mux(match (read, write) {
        (true, true) => {
            ...
            redox_rt::sys::sys_call_rw(fd, payload, flags, metadata)
        }
        (true, false) => {
            // read-only
            let payload = unsafe { slice::from_raw_parts_mut(payload, payload_len) };
            redox_rt::sys::sys_call_ro(fd, payload, flags, metadata)
        }
        (false, true) => {
            // write-only
            let payload = unsafe { slice::from_raw_parts(payload.cast_const(), payload_len) };
            redox_rt::sys::sys_call_wo(fd, payload, flags, metadata)
        }
        (false, false) => {
            // no payload
            redox_rt::sys::sys_call(fd, flags, metadata)
        }
    })
}
...
pub unsafe extern "C" fn redox_sys_call_multiple_v0(
    ...
    Error::mux(match (read, write) {
        (true, true) => {
            // read-write
            let payload = unsafe { slice::from_raw_parts_mut(payload, payload_len) };
            redox_rt::sys::sys_call_rw(fds, payload, flags, metadata)
        }
        (true, false) => {
            // read-only
            let payload = unsafe { slice::from_raw_parts_mut(payload, payload_len) };
            redox_rt::sys::sys_call_ro(fds, payload, flags, metadata)
        }
        (false, true) => {
            // write-only
            let payload = unsafe { slice::from_raw_parts(payload.cast_const(), payload_len) };
            redox_rt::sys::sys_call_wo(fds, payload, flags, metadata)
        }
        (false, false) => {
            // no payload
            redox_rt::sys::sys_call(fds, flags, metadata)
        }
    })
}
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn redox_numa_set_mem_policy_v0(policy: *const u8, len: usize) -> RawResult {
    Error::mux(unsafe { set_mem_policy(slice::from_raw_parts(policy, len)).map(|()| 0) })
}

#[unsafe(no_mangle)]
pub unsafe extern "C" fn redox_numa_get_mem_policy_v0(policy: *mut u8, len: usize) -> RawResult {
    Error::mux(unsafe { get_mem_policy(slice::from_raw_parts_mut(policy, len)).map(|()| 0) })
}
