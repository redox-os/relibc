        ...
        while unsafe { *format } != 0 {
            ...
            match unsafe { *format } as u8 {
                ...
                b'a' => {
                    ...
                    let s = unsafe { langinfo_to_str(ABDAY_1 + t_val.tm_wday as i32) };
                    w!(s);
                }
                ...
                b'A' => {
                    ...
                    let s = unsafe { langinfo_to_str(DAY_1 + t_val.tm_wday as i32) };
                    w!(s);
                }

                // Abbreviated month name: %b or %h
                b'b' | b'h' => {
                    let s = unsafe { langinfo_to_str(ABMON_1 + t_val.tm_mon as i32) };
                    w!(s);
                }

                // Full month name: %B
                b'B' => {
                    let s = unsafe { langinfo_to_str(MON_1 + t_val.tm_mon as i32) };
                    w!(s);
                }

                // Century: %C
                b'C' => {
                    let mut year = t_val.tm_year / 100;
                    if t_val.tm_year % 100 != 0 {
                        year += 1;
                    }
                    w!("{:02}", year + 19);
                }
                ...
                // Day of month: %d
                b'd' => w!("{:02}", t_val.tm_mday),

                // %D => same as %m/%d/%y
                b'D' => w!(recurse "%m/%d/%y"),

                // Day of month, space-padded: %e
                b'e' => w!("{:2}", t_val.tm_mday),

                // ISO 8601 date: %F => %Y-%m-%d
                b'F' => w!(recurse "%Y-%m-%d"),

                // Hour (00-23): %H
                b'H' => w!("{:02}", t_val.tm_hour),

                // Hour (01-12): %I
                b'I' => w!("{:02}", (t_val.tm_hour + 12 - 1) % 12 + 1),

                // Day of year: %j
                b'j' => w!("{:03}", t_val.tm_yday),

                // etc.
                b'k' => w!("{:2}", t_val.tm_hour),
                b'l' => w!("{:2}", (t_val.tm_hour + 12 - 1) % 12 + 1),
                b'm' => w!("{:02}", t_val.tm_mon + 1),
                b'M' => w!("{:02}", t_val.tm_min),
                ...
                b'S' => w!("{:02}", t_val.tm_sec),

                // 24-hour clock with seconds: %T => %H:%M:%S
                b'T' => w!(recurse "%H:%M:%S"),

                // Weekday (1-7, Monday=1): %u
                b'u' => w!("{}", (t_val.tm_wday + 7 - 1) % 7 + 1),

                // Sunday-based week of year: %U
                b'U' => w!("{}", (t_val.tm_yday + 7 - t_val.tm_wday) / 7),

                // Weekday (0-6, Sunday=0): %w
                b'w' => w!("{}", t_val.tm_wday),

                // Monday-based week of year: %W
                b'W' => w!("{}", (t_val.tm_yday + 7 - (t_val.tm_wday + 6) % 7) / 7),

                // Last two digits of year: %y
                b'y' => w!("{:02}", t_val.tm_year % 100),

                // Full year: %Y
                b'Y' => w!("{}", t_val.tm_year + 1900),
