    ...
    if unsafe { unistd::pipe2(p.as_mut_ptr(), fcntl::O_CLOEXEC) } != 0 {
        ...
        if pid == 0 {
            ...
            if unsafe { utmp::login_tty(s) } != 0 {
                ...
                let err_size = mem::size_of_val(err);
                unsafe { unistd::write(p[1], err.as_ptr().cast(), err_size) };
                unistd::_exit(127);
            }
            unistd::close(p[1]);
            unsafe {
        ...
        let ec_ptr = &mut ec as *mut c_int as *mut c_void;
        if unsafe { unistd::read(p[0], ec_ptr, mem::size_of::<c_int>()) } > 0 {
            let mut status = 0;
