...
pub unsafe extern "C" fn pthread_kill(thread: pthread_t, sig: c_int) -> c_int {
    let os_tid = {
        ...
        let os_tid_ptr = pthread.os_tid.get();
        unsafe { os_tid_ptr.read() }
    };
...
pub unsafe extern "C" fn sigaction(
    ...
    let oact = unsafe { oact.as_mut() };
    Sys::sigaction(sig, act, oact)
        .map(|()| 0)
        .or_minus_one_errno()
}
...
pub unsafe extern "C" fn sigaltstack(ss: *const stack_t, old_ss: *mut stack_t) -> c_int {
    let ss = unsafe { ss.as_ref() };
    let old_ss = unsafe { old_ss.as_mut() };
    unsafe { Sys::sigaltstack(ss, old_ss) }
        .map(|()| 0)
        .or_minus_one_errno()
}
...
pub unsafe extern "C" fn sigemptyset(set: *mut sigset_t) -> c_int {
    let set = unsafe { (set as *mut SigSet).as_mut() };
    if let Some(set) = set {
        set.clear();
    }
    0
...
pub extern "C" fn sigignore(sig: c_int) -> c_int {
    ...
    let sa_ref = unsafe { &mut *psa.as_mut_ptr() };
    unsafe { sigemptyset(&mut sa_ref.sa_mask) };
...
pub unsafe extern "C" fn sigset(
    ...
    let old_sa = unsafe { old_sa.assume_init() };
    old_sa.sa_handler
}
...
pub unsafe extern "C" fn sigtimedwait(
    ...
    let set = unsafe { &*set };
    let sig = unsafe { sig.as_mut() };
    let tp = unsafe { tp.as_ref() };
...
pub unsafe extern "C" fn psignal(sig: c_int, prefix: *const c_char) {
    ...
    let string_ptr = string.as_bytes().as_ptr().cast();
    let string_len = string.as_bytes().len();
    // TODO: better internal libc API?
    let _ = unsafe { unistd::write(unistd::STDERR_FILENO, string_ptr, string_len) };
}
#[no_mangle]
