...
pub unsafe extern "C" fn atexit(func: Option<extern "C" fn()>) -> c_int {
    let atexit_funcs = unsafe { &mut ATEXIT_FUNCS };
...
unsafe extern "C" fn void_cmp(a: *const c_void, b: *const c_void) -> c_int {
    unsafe { *(a as *const i32) - *(b as *const i32) as c_int }
}
...
pub unsafe extern "C" fn exit(status: c_int) -> ! {
    ...
    let atexit_func = unsafe { &ATEXIT_FUNCS };
    for i in (0..atexit_func.len()).rev() {
        if let Some(func) = atexit_func[i] {
            (func)();
        }
    }
...
unsafe fn find_env(search: *const c_char) -> Option<(usize, *mut c_char)> {
    for (i, mut item) in platform::environ_iter().enumerate() {
        ...
        loop {
            ...
            assert_ne!(unsafe { *item }, 0, "environ has an item without value");
...
pub unsafe extern "C" fn getsubopt(
    ...
    let max = unsafe { strlen(start) } as isize;
    let mut i: usize = 0;
    unsafe { *valuep = ptr::null_mut() };
    let start_comma = unsafe { strchr(start, b',' as i32) };
    unsafe { *optionp = start_comma };
    if !unsafe { *optionp }.is_null() {
        unsafe {
            *(*optionp).add(0) = 0;
            *optionp = *optionp.add(1);
        }
    } else {
        unsafe { *(optionp) = start.add(max as usize) };
    }
    while !unsafe { tokens.offset(i as isize) }.is_null() {
        let cur = unsafe { tokens.offset(i as isize) } as *const _;
        let len = unsafe { strlen(cur) } as isize;
        if unsafe { strncmp(cur, start, len as usize) } != 0 {
            i = i + 1;
            continue;
        }
        if unsafe { *start.offset(len) } == b'=' as i8 {
            unsafe { *valuep = start.offset(len + 1) };
        } else if !unsafe { start.offset(len) }.is_null() {
            i = i + 1;
            continue;
        }
        return i as c_int;
...
pub unsafe extern "C" fn l64a(value: c_long) -> *mut c_char {
    ...
    let l64a_buffer = unsafe { &mut L64A_BUFFER };
...
pub unsafe extern "C" fn ptsname(fd: c_int) -> *mut c_char {
    ...
    let pts_buffer_ptr = unsafe { PTS_BUFFER.as_mut_ptr() };
    let pts_buffer_len = unsafe { PTS_BUFFER.len() };
    if unsafe { ptsname_r(fd, pts_buffer_ptr, pts_buffer_len) } != 0 {
        ptr::null_mut()
    } else {
        pts_buffer_ptr
    }
...
unsafe fn put_new_env(insert: *mut c_char) {
    ...
    let environ = unsafe { &mut platform::environ };
    let our_environ = unsafe { &mut platform::OUR_ENVIRON };
...
pub unsafe extern "C" fn putenv(insert: *mut c_char) -> c_int {
    ...
    if let Some((i, _)) = unsafe { find_env(insert) } {
        ...
        unsafe { environ.add(i).write(insert) };
    } else {
...
pub unsafe extern "C" fn rand_r(seed: *mut c_uint) -> c_int {
    if seed.is_null() {
        ...
        const size: usize = 16 / mem::size_of::<c_uint>();
        let seed_arr: [u8; 16] = unsafe { mem::transmute([*seed; size]) };
...
pub unsafe extern "C" fn random() -> c_long {
    ...
    if random_state.n == 0 {
        ...
        let x_new_bytes = x_new.to_ne_bytes();
        unsafe { *random_state.x_ptr = x_new_bytes };
        k = x_new;
    } else {
        let random_state_i = usize::from(random_state.i);
        let random_state_j = usize::from(random_state.j);
        // The non-u32-aligned way of saying x[i] += x[j]...
        let x_i_old = u32::from_ne_bytes(unsafe { *random_state.x_ptr.add(random_state_i) });
        let x_j = u32::from_ne_bytes(unsafe { *random_state.x_ptr.add(random_state_j) });
        let x_i_new = x_i_old.wrapping_add(x_j);
        let x_i_new_bytes = x_i_new.to_ne_bytes();
        unsafe { *random_state.x_ptr.add(random_state_i) = x_i_new_bytes };
...
unsafe fn copy_kv(
    ...
    let existing = unsafe { existing.add(key_len) };

    unsafe { core::ptr::write(existing, b'=' as c_char) };
    let existing = unsafe { existing.add(1) };

    unsafe { core::ptr::copy_nonoverlapping(value, existing, value_len) };
    let existing = unsafe { existing.add(value_len) };

    unsafe { core::ptr::write(existing, 0) };
}
...
pub unsafe extern "C" fn setenv(
    ...
    if let Some((i, existing)) = unsafe { find_env(key) } {
        ...
        if existing_len >= value_len {
            ...
            let existing_next = unsafe { existing.add(value_len) };
            unsafe { core::ptr::write(existing_next, 0) };
        } else {
...
pub unsafe extern "C" fn srand(seed: c_uint) {
    let rng = XorShiftRng::from_seed([seed as u8; 16]);
    unsafe { RNG = Some(rng) };
}
...
pub unsafe fn convert_octal(s: *const c_char) -> Option<(c_ulong, isize, bool)> {
    if unsafe { *s } != 0 && unsafe { *s } == b'0' as c_char {
        let s_off = unsafe { s.offset(1) };
        if let Some((val, idx, overflow)) = unsafe { convert_integer(s_off, 8) } {
    ...
    if (unsafe { *s } != 0 && unsafe { *s } == b'0' as c_char)
        && (unsafe { *s.offset(1) } != 0
            && (unsafe { *s.offset(1) } == b'x' as c_char
                || unsafe { *s.offset(1) } == b'X' as c_char))
    {
        let s_off = unsafe { s.offset(2) };
        unsafe { convert_integer(s_off, 16) }.map(|(val, idx, overflow)| (val, idx + 2, overflow))
    } else {
...
pub unsafe extern "C" fn strtof(s: *const c_char, endptr: *mut *mut c_char) -> c_float {
    unsafe { strto_float_impl!(c_float, s, endptr) }
}
...
pub unsafe extern "C" fn strtol(s: *const c_char, endptr: *mut *mut c_char, base: c_int) -> c_long {
    const clong_max: c_long = c_long::max_value();
    const clong_min: c_long = c_long::min_value();
    unsafe { strto_impl!(c_long, true, clong_max, clong_min, s, endptr, base) }
}
...
pub unsafe extern "C" fn strtoll(
    ...
    const clonglong_max: c_longlong = c_longlong::max_value();
    const clonglong_min: c_longlong = c_longlong::min_value();
    unsafe {
        strto_impl!(
            c_longlong,
            true,
...
pub unsafe extern "C" fn strtoul(
    ...
    const culong_max: c_ulong = c_ulong::max_value();
    const culong_min: c_ulong = c_ulong::min_value();
    unsafe { strto_impl!(c_ulong, false, culong_max, culong_min, s, endptr, base) }
}
...
pub unsafe extern "C" fn strtoull(
    ...
    const culonglong_max: c_ulonglong = c_ulonglong::max_value();
    const culonglong_min: c_ulonglong = c_ulonglong::min_value();
    unsafe {
        strto_impl!(
            c_ulonglong,
            false,
...
pub unsafe extern "C" fn system(command: *const c_char) -> c_int {
    ...
    if child_pid == 0 {
        ...
        let mut wstatus = 0;
        if unsafe { Sys::waitpid(child_pid, &mut wstatus, 0) }.or_minus_one_errno() == -1 {
            return -1;
        }
