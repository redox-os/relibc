...
pub unsafe extern "C" fn pthread_barrier_init(
    ...
    let barrier_value = RlctBarrier::new(count);
    unsafe { barrier.cast::<RlctBarrier>().write(barrier_value) };
    0
}
...
pub unsafe extern "C" fn pthread_barrierattr_init(attr: *mut pthread_barrierattr_t) -> c_int {
    let barrier_value = RlctBarrierAttr::default();
    unsafe { core::ptr::write(attr.cast::<RlctBarrierAttr>(), barrier_value) };
...
pub unsafe extern "C" fn pthread_barrierattr_setpshared(
    ...
    let attr = unsafe { &mut *attr.cast::<RlctBarrierAttr>() };
...
pub unsafe extern "C" fn pthread_barrierattr_getpshared(
    ...
    let attr = unsafe { &*attr.cast::<RlctBarrierAttr>() };
    unsafe { core::ptr::write(pshared, attr.pshared) };
    0
}
