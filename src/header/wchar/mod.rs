...
pub unsafe extern "C" fn fgetwc(stream: *mut FILE) -> wint_t {
    ...
    loop {
        let buf_ptr = buf[bytes_read..bytes_read + 1].as_mut_ptr();
        let nread = unsafe { fread(buf_ptr as *mut c_void, 1, 1, stream) };
...
pub unsafe extern "C" fn fputwc(wc: wchar_t, stream: *mut FILE) -> wint_t {
    ...
    let amount = unsafe { wcrtomb(bytes_ptr, wc, internal) };
...
pub unsafe extern "C" fn mbrlen(s: *const c_char, n: size_t, ps: *mut mbstate_t) -> size_t {
    ...
    unsafe { mbrtowc(ptr::null_mut(), s, n, internal) }
}
...
pub unsafe extern "C" fn mbsnrtowcs(
    ...
    while (dst_ptr.is_null() || dst_offset < dst_len) && src_offset < src_len {
        ...
        let mut wc: wchar_t = 0;
        let src_ptr_off = unsafe { src.add(src_offset) };
        let amount = unsafe { mbrtowc(&mut wc, src_ptr_off, src_len - src_offset, ps) };

        // Stop in the event a decoding error occured.
        if amount == -1isize as usize {
            unsafe { *src_ptr = src_ptr_off };
...
pub unsafe extern "C" fn putwchar(wc: wchar_t) -> wint_t {
    let stdout_ref = unsafe { &mut *stdout };
    unsafe { fputwc(wc, stdout_ref) }
}
...
pub unsafe extern "C" fn swscanf(
    ...
    unsafe { vswscanf(s, format, va_list) }
}
...
pub unsafe extern "C" fn ungetwc(wc: wint_t, stream: &mut FILE) -> wint_t {
    ...
    let amount = unsafe { wcrtomb(bytes_ptr, wc as wchar_t, internal) };
    if amount == usize::MAX {
        return WEOF;
    }
    ...
    */
    for i in 0..amount {
        unsafe { ungetc(bytes[amount - 1 - i] as c_int, stream) };
    }
...
pub unsafe extern "C" fn vfwprintf(
    ...
    unsafe { wprintf::wprintf(&mut *stream, format, arg) }
}
#[no_mangle]
...
pub unsafe extern "C" fn wcspbrk(mut wcs: *const wchar_t, set: *const wchar_t) -> *mut wchar_t {
    let len = unsafe { wcscspn(wcs, set) };
    wcs = unsafe { wcs.add(len) };
    if unsafe { *wcs } == 0 {
        ptr::null_mut()
    } else {
...
pub unsafe extern "C" fn wcstok(
    ...
    let len = unsafe { wcsspn(wcs, delim) };
    wcs = unsafe { wcs.add(len) };
...
pub unsafe extern "C" fn vwscanf(format: *const wchar_t, __valist: va_list) -> c_int {
    let mut file = unsafe { &mut *stdin }.lock();
